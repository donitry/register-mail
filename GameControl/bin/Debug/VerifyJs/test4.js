...
(function(g, n) {
	...
	function p(c) {
		return (c = document.cookie.match(RegExp("(?:^|;\\s)" + c + "=(.*?)(?:;\\s|$)"))) ? c[1] : ""
	}
	...
	function l(c) {
		...
		if (b) for (var h = b.split("&"), f = 0, g = h.length; f < g; f++)
		if (-1 != h[f].indexOf("=")) {
			var m = h[f].indexOf("="),
				k = h[f].slice(0, m),
				m = h[f].slice(m + 1);
	...
	function y() {
		try {
			var c = navigator,
				...
				e = a.colorDepth + "-bit",
				h = (c.language || c.userLanguage).toLowerCase(),
				f = c.javaEnabled() ? 1 : 0,
	...
	function z() {
		var c = {};
		if ("undefined" != typeof _taadHolders && 0 < _taadHolders.length) for (var a = 0, b = _taadHolders, d = b.length; a < d; a++)
